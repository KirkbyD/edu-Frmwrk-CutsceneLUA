...
#pragma region PATTERNS_FINAL_BOOLEANS
bool Q3CameraMove = false;
bool Q4LineUp = false;
bool Q5Cannonball = false;
bool Q6Fleeing = false;
bool Q7Sunk = false;
#pragma endregion
...
int main() {
	...
	//else {
		...
		isObjCtrl = false;
	//}
	FreeCamera.SetEye(glm::vec3(-387.0f, 209.0f, 533.2f));
	FreeCamera.SetTarget(pFindObjectByFriendlyName("0 Test"));
	...
	pLuaManager->ExecuteScript(mpScripts["FINALQ3"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ4"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ4Trigger"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ4CamTrigger"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ6Camera"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ6Defender"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ6Attacker"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ7Defender"]);
	pLuaManager->ExecuteScript(mpScripts["FINALQ7Attacker"]);
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		// SCRIPTED COMMAND CALLS
		//if (!pSceneCG->IsDone())
		//{
		/*if (!isDevCon) {
			if(!pCommandManager->_map_script_to_cmd["TestCameraScript"]->Update(deltaTime)) { }
			else { }
		}*/
		// set up a global if statement for EACH command I want to run
		// will run once per load cause popoffs
		// set bool != update() in ifs
		if (Q3CameraMove) {
			Q3CameraMove = !pCommandManager->_map_script_to_cmd["FINALQ3"]->Update(deltaTime);
		}
		if (Q4LineUp) {
			Q4LineUp = !pCommandManager->_map_script_to_cmd["FINALQ4"]->Update(deltaTime);
			pCommandManager->_map_script_to_cmd["FINALQ4Trigger"]->Update(deltaTime);
			pCommandManager->_map_script_to_cmd["FINALQ4CamTrigger"]->Update(deltaTime);
		}
		if (Q5Cannonball) {
			//Q5Cannonball = !pCommandManager->_map_script_to_cmd["FINALQ5"]->Update(deltaTime);
		}
		if (Q6Fleeing) {
			pCommandManager->_map_script_to_cmd["FINALQ6Camera"]->Update(deltaTime);
			pCommandManager->_map_script_to_cmd["FINALQ6Defender"]->Update(deltaTime);
			Q6Fleeing = !pCommandManager->_map_script_to_cmd["FINALQ6Attacker"]->Update(deltaTime);
		}
		if (Q7Sunk) {
			pCommandManager->_map_script_to_cmd["FINALQ7Attacker"]->Update(deltaTime);

			Q7Sunk = !pCommandManager->_map_script_to_cmd["FINALQ7Defender"]->Update(deltaTime);
		}
